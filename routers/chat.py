...
async def registration(response: Response, request: Request, user: Post_Session):
    ...
    answer = requests.get(f'https://ipinfo.io/{request.client.host}/json')
    if not answer.ok:
...
async def get_chats(session: Optional[str] = Cookie(default=None)):
    ...
    chats = []
    for chat in query.user.chats:
        chats.append(Return_chat(chat_id=chat.chat_id, username=chat.username, chat_type=chat.chat_type, avatar=chat.avatar, title=chat.title, last_message=chat.last_message))
...
async def get_messages(chat: Post_chat, session: Optional[str] = Cookie(default=None)):
    if not session:
        return {'ok': False, 'detail': 'Not authorizated'}
...
async def download_file(chat_id: str, file_path: str, session = Cookie(default=None)):
...
async def create_chat(session = Cookie(default=None), image: Optional[UploadFile] = File(None), chat_name: str = Form(), chat_username: str = Form(), chat_type: str = Form(), chat_desc: str = Form(None)):
    ...
    if image: #saving image
        ...
        image_path = os.path.join('attach_files', str(chat_id), file_name_db)

        with open(image_path, 'wb') as buffer:
            shutil.copyfileobj(image.file, buffer)
    
    new_chat = sql.Chat(chat_id=chat_id, username=chat_username, title=chat_name, type=sql.Chat_type.group if chat_type == 'group' else sql.Chat_type.channel, description=chat_desc, created_by=query.user_id, avatar=image_path if image else None)
